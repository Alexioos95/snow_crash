...
Nothing... and there is nothing left to exploit too. Unless... Time to exploit `getflag` itself.
...
It has a lot of if/else if/else, that must be the different cases of which we call `getflag`. We see `ft_des`, and the string from the previous levels, so we could do the same as the previous exercice, but it's funnier if we use `gdb` to make the call.
...
Wow, that's a lot... but with the help of `Ghidra`, we can try to navigate in blocks. The one of the last `fputs` is this one:
...
After trying to `jump` to a few of these addresses...

	(gdb) start
	The program being debugged has been started already.
	Start it from the beginning? (y or n) y
	Temporary breakpoint 11 at 0x804894a
	Starting program: /bin/getflag 

	Temporary breakpoint 11, 0x0804894a in main ()
	(gdb) jump *0x08048de5
...
We got `7QiHafiNa3HVozsaXkawuYrTstxbpABHD8CPnHJ`, which, somehow, do allow us to log into `flag14`.
